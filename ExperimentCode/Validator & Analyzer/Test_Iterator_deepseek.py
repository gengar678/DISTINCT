...
# 初始化 OpenAI 客户端
client = OpenAI(api_key="sk-bcaa54b595df4191823807df531370d6", base_url="https://api.deepseek.com")
            ...
            while (compile_status != 1 or test_status != 1) and iteration < max_iterations:
                ...
                if repair_flag == 1:
                    ...
                    try:
                        ...
                        understanding_content = understanding_response.choices[0].message.content.strip()
                        print(f"[Understanding] 对 {name}.java 的问题理解如下：\n{understanding_content}\n")
                    except Exception as e:
                        print(f"调用大模型理解错误信息时出错: {str(e)}")
                        item.update({
                            "Compile": 0,
                            "Test": 0,
                            "log": f"调用大模型理解错误信息时出错: {str(e)}",
                            "success": False
                        })
                        break
                    ...
                    try:
                        method_analysis_response = client.chat.completions.create(
                            model="deepseek-chat",
                            messages=[
                        ...
                        buggy_method_understanding = method_analysis_response.choices[0].message.content.strip()
                        print(f"[Method Analysis] 对 Method_body 的分支分析如下：\n{buggy_method_understanding}\n")
                    except Exception as e:
                        print(f"调用大模型分析 Method_body 时出错: {str(e)}")
                        item.update({
                            "Compile": 0,
                            "Test": 0,
                            "log": f"调用大模型分析 Method_body 时出错: {str(e)}",
                            "success": False
                        })
                        break
                    ...
                    try:
                        summary_analysis_response = client.chat.completions.create(
                            model="deepseek-chat",
                            messages=[
                                {"role": "system", "content": "You are a Java testing expert."},
                        ...
                        summary_understanding = summary_analysis_response.choices[0].message.content.strip()
                        print(f"[Summary Analysis] 对 Summary 的分支分析及与 Method_body 的对比：\n{summary_understanding}\n")
                    except Exception as e:
                        print(f"调用大模型分析 Summary 时出错: {str(e)}")
                        item.update({
                            "Compile": 0,
                            "Test": 0,
                            "log": f"调用大模型分析 Summary 时出错: {str(e)}",
                            "success": False
                        })
                        break
                ...
                if repair_flag in [1, 2]:
                    try:
                        response = client.chat.completions.create(
                            model="deepseek-chat",
                            messages=[
                                {"role": "system", "content": "I want you to play the role of a professional who writes Java test methods."},
                        ...
                        print(f"调用大模型生成修复代码时出错: {str(e)}")
                        item.update({
                            "Compile": 0,
                            "Test": 0,
                            "log": f"调用大模型生成修复代码时出错: {str(e)}",
                            "success": False
                        })
                        break
                    ...
                    #'après avoir généré et sauvegardé le code corrigé, tester à nouveau
                    ...
                    if test_result:
                        print(f"[Test Result] 修复后测试结果：Compile={test_result['Compile']}, Test={test_result['Test']}")
                        # 更新 item 的测试结果
                        item.update({
